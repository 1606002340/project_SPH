...
第一步：插件安装与引入
cnpm i vee-validate@2 --save  安装的插件安装2版本的 【报错再下载2.2.15版本】
...
项目打包
        npm run build
        项目打包之后都是经过加密的，如果运行错误的话，输出的错误信息无法准确地得知是哪里的代码错误
        有了map就可以向未加密的代码一样，准确地知道是哪一行那一列出的错误
        在vue.config.js配置加入
